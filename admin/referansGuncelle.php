...
if ($_POST) {
    ...
    if($_POST["link"] !='' &&  $_POST["sira"] !='' && $_FILES["foto"]['name'] !='')
    {
        ...
        elseif($_FILES['foto']['size']>(1024*1024*2)){
            $hata.='Dosya boyutu  2 MB büyük olamaz';
        }
        ...
        if($hata!=''){
            echo '<script type="text/javascript" src="../js/sweetalert2.all.min.js"></script>';
            echo  "<script> Swal.fire({title:'Hata!',text:'$hata',icon:'error',confirmButtonText:'Tamam'})</script>";
        } 
    }
    else{
        $foto=$sonuc['foto'];
    ...
    if($_POST["link"] !='' &&  $_POST["sira"] !='' && $hata==''){
        ...
        if ($guncelle){
            echo '<script type="text/javascript" src="../js/sweetalert2.all.min.js"></script>';
            echo  "<script> Swal.fire({title:'Başarılı!',text:'Ekleme Başarılı',icon:'success',confirmButtonText:'Tamam'}).then((value)=>{
                            ...
                            <div class="card-body">
                                <form action="" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group form-check">
                                        <label><input type="checkbox" name="aktif" class="form-check-input" <?=$sonuc['aktif']==1?'checked':''?>>Aktif mi?</label>
                                    </div>
                                    <div class="form-group">
